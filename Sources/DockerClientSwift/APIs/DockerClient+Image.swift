...
extension DockerClient {
    ...
    public struct ImagesAPI {
        ...
        public func pullImage(byName name: String, tag: String? = nil, digest: Digest? = nil)
            async throws -> Image
        {
            var identifier = name
            if let tag = tag {
        ...
        /// Pulls an image by a given identifier.
        ...
        /// - Parameter all: If `true` intermediate image layers will be returned as well. Default is `false`.
        /// - Throws: Errors that can occur when executing the request.
        ...
        ///   - force: Should the image be removed by force? Default is `false`.
        /// - Throws: Errors that can occur when executing the request.
        public func remove(image: Image, force: Bool = false) async throws {
            _ = try await client.run(RemoveImageEndpoint(imageId: image.id.value, force: force))
        }
